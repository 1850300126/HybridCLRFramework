    ...
    public class FlowLayoutGroup : LayoutGroup
    {
        ...
        private void SetCellsAlongAxis()
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                layout = rectChild.TryGetComponent<LayoutElement>(out var layoutElement)
                    ? layoutElement
                    : rectChild.gameObject.AddComponent<LayoutElement>();

                SetChildAlongAxis(rectChild, 0, startOffset.x + totalWidth, layout.preferredWidth);
                SetChildAlongAxis(rectChild, 1, startOffset.y + totalHeight, layout.preferredHeight);

                currentSpacing = spacing;

                if (horizontal)
                {
                    totalWidth += rectChild.rect.width + currentSpacing[0];
                    if (rectChild.rect.height > lastMax)
                    {
                        lastMax = rectChild.rect.height;
                    }
                ...
                {
                    totalHeight += rectChild.rect.height + currentSpacing[1];
                    if (rectChild.rect.width > lastMax)
                    {
                        lastMax = rectChild.rect.width;
                    }
